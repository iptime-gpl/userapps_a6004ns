...
spiflash_init(si_t *sih)
{
	...
	case NXPART:
		...
		case 0x18:
			/* ST M25FL128 128 Mbit Serial Flash */
                        if( vendor_id == MACRONIXPART || vendor_id == NXPART)
	...
	if (BCM4707_CHIP(CHIPID(sih->chip))) {
		...
		{
			printf("Force 3 Bytes Mode --> CHIPCB_CRU_STRAPS_4BYTE\n");
			force_3byte_mode = 1;
		}
	}
	...
	if (force_3byte_mode && (spiflash.size > SI_FLASH_WINDOW)) {
		...
		printf("1) NOR flash size %dMB is bigger than %dMB, limit it to %dMB\n",
			(spiflash.size >> 20), (SI_FLASH_WINDOW >> 20),
			(SI_FLASH_WINDOW >> 20));
		...
		printf("2) NOR flash size %dMB is bigger than %dMB, limit it to %dMB\n",
			(spiflash.size >> 20), (SI_NS_FLASH_WINDOW >> 20),
			(SI_NS_FLASH_WINDOW >> 20));
