...
nandcore_override_config(hndnand_t *nfl)
{
	...
	/*  CONFIG_EFM_PATCH : Important for A6004NS to support NAND 2K flash without HW bootstrap change */
	reg = R_REG(osh, &nc->config_cs0);
	reg &= ~(NANDCF_CS0_PAGE_SIZE_MASK);
	reg |= 0x00100000; /* Fix to 2K */
	W_REG(osh, &nc->config_cs0, reg);
	/*  CONFIG_EFM_PATCH : End */
}
...
#define NAND_BLOCK_SIZE 0x40000 /* MAX 256K */
char buf2[NAND_BLOCK_SIZE];
#endif
...
nandcore_write(hndnand_t *nfl, uint64 offset, uint len, const uchar *buf)
{
	...
	while (res > 0) {
		...
		if (ret < 0)
		{
			printk("--> nandcore_write error :  offset -> %llu res -> %d\n", offset, res );
			return ret;
		}
...
#ifdef CONFIG_EFM_PATCH
	{
		if(len <= NAND_BLOCK_SIZE)
		{
			nandcore_read(nfl, offset_o, len, buf2);
			if(memcmp(buf,buf2,len))
			{
				printk("NAND BAD Block Found:0x%08x\n", (uint32)offset&0xffffffff);
				return -1;
			}
		}
...
nandcore_erase(hndnand_t *nfl, uint64 offset)
{
	...
	{
printk("--> nandcore_erase error 1\n");
		return -1;
	}
	if ((offset & (nfl->blocksize - 1)) != 0) {
printk("--> nandcore_erase error 2\n");
		return -1;
	}
