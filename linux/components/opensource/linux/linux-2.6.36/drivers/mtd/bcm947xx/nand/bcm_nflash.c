...
static int proc_read_nandinfo(char *buffer, char **start, off_t offset, int length, int *eof, void *data)
{
        char *p = buffer;
        int len,i;
	int badblock_count=0;
	
	for(i=0;i< nflash.region.numblocks;i++)
		if(nflash.map[i]) badblock_count++;

	p += sprintf(p,"BadBlock:%d(%d bytes)/%d(%d bytes)\n", 
			badblock_count,
			badblock_count*nflash.region.erasesize,
			nflash.region.numblocks,
			nflash.region.numblocks*nflash.region.erasesize);

	p += sprintf(p,"Bad Block MAP(X=badblock):\n");
	for(i=0;i<nflash.region.numblocks;i++)
	{
		p += sprintf(p,"%s",nflash.map[i]?"X":"O");
		if(!((i+1)%64)) p += sprintf(p,"\n");
	}
	p += sprintf(p,"\n");
...
static int proc_write_nandinfo(struct file *file, const char __user *buf,
...
nflash_mtd_init(void)
{
	...
	/* Scan bad block */
printk("NAND FLASH BAD BLOCK MAP\n");
	NFLASH_LOCK(&nflash);
	for (i = 0; i < info->numblocks; i++) {
		if (hndnand_checkbadb(nflash.nfl, (i * (uint64)info->blocksize)) != 0) {
			nflash.map[i] = 1;
printk("NAND Block #%d -> BAD Block\n",i);
			badcount++;
		}
	}
	NFLASH_UNLOCK(&nflash);
printk("Summary BAD Block : %d / TOTAL Block : %d\n", badcount, info->numblocks );
	...
	nflash_parts = parts;
        ...
        sprintf(name, "nandinfo");
        proc_entry = create_proc_entry(name,0,0);
        if(proc_entry)
	{
                proc_entry->read_proc=&proc_read_nandinfo;
                proc_entry->write_proc=&proc_write_nandinfo;
	}
#endif
	return 0;
