...
#ifdef CONFIG_EFM_PATCH_FOR_NAND
                {
                        char *bootfrom = nvram_get("bootfrom");
                        printk(KERN_INFO "BOOTFROM ---> %s\n", bootfrom?bootfrom:"spi");
                        if(bootfrom && !strcmp(bootfrom,"nand"))
                                return 9; /* because of ftm partition */
                        return 2;
                }
#else
			return 2;
...
#define FLASH_PARTS_NUM 7 /* Config + FTM */
#else
...
init_mtd_partitions(hndsflash_t *sfl_info, struct mtd_info *mtd, size_t size)
{
	...
	if (knldev != SOC_KNLDEV_NANDFLASH) {
		...
		bcm947xx_flash_parts[nparts].size -= ROUNDUP(nvram_space, mtd->erasesize);
#ifdef CONFIG_EFM_PATCH
                bcm947xx_flash_parts[nparts].size -= 0x10000; /* Reserve Config Size */
                bcm947xx_flash_parts[nparts].size -= 0x10000; /* Reserve ftm flag region */
#endif
	...
	nparts++;
        ...
        bcm947xx_flash_parts[nparts].name = "ftm";
        bcm947xx_flash_parts[nparts].size = ROUNDUP(nvram_space, mtd->erasesize);
        bcm947xx_flash_parts[nparts].offset = size - bcm947xx_flash_parts[nparts].size * 3;
        nparts++;
#endif
...
#ifdef CONFIG_EFM_PATCH_FOR_NAND
        /* Setup kernel MTD partition */
        bcm947xx_nflash_parts[nparts].name = "nlinux";
        /* bcm947xx_nflash_parts[nparts].size = 0x2000000; */
        bcm947xx_nflash_parts[nparts].size = size;
        bcm947xx_nflash_parts[nparts].offset = 0x0;

        {
                char *bootfrom = nvram_get("bootfrom");
                printk(KERN_INFO "NAND MTD INIT  BOOTFROM ---> %s\n", bootfrom?bootfrom:"spi");
                if(bootfrom && !strcmp(bootfrom,"nand"))
                {
                        printk("Lookup Nflash ROOTFS\n");
                        shift = lookup_nflash_rootfs_offset(nfl, mtd, offset, bcm947xx_nflash_parts[nparts].size);
                        nparts++;
                        bcm947xx_nflash_parts[nparts].name = "nrootfs";
                        /* bcm947xx_nflash_parts[nparts].size = 0x2000000 - shift - 0x50000; *//* 0x40000 boot :  0x10000 : config */
                        bcm947xx_nflash_parts[nparts].size = size - shift - 0x50000; /* 0x40000 boot :  0x10000 : config */
                        bcm947xx_nflash_parts[nparts].offset = shift;
                        bcm947xx_nflash_parts[nparts].mask_flags = MTD_WRITEABLE;
                        nparts++;
                }
...
#if     0
        bcm947xx_nflash_parts[nparts].name = "ffs";
        bcm947xx_nflash_parts[nparts].size = size - 0x2000000;
        bcm947xx_nflash_parts[nparts].offset = 0x2000000;
        bcm947xx_nflash_parts[nparts].mask_flags = 0;
        bcm947xx_nflash_parts[nparts].ecclayout = mtd->ecclayout;
        nparts++;

        bcm947xx_nflash_parts[nparts].name = "nall";
        bcm947xx_nflash_parts[nparts].size = 0;
        bcm947xx_nflash_parts[nparts].offset = 0;
        bcm947xx_nflash_parts[nparts].mask_flags = 0;
        nparts++;
#endif
