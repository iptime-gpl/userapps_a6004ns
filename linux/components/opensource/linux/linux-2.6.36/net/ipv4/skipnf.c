...
int check_skip_packet(int direction,struct sk_buff *skb)
{
	...
	int i,port;
        ...
        {
		//printk("port -> %04x skip_list[i].port -> %04x\n", port, skip_list[i].port);
                if(!skip_list[i].port) return 0;
                if((skip_list[i].port == htons(port)) && (!strcmp(skip_list[i].ifname,"all") || !strcmp(skip_list[i].ifname,dev->name)))
		{
