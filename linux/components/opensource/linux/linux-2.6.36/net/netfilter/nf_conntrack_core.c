...
#ifdef CONFIG_EFM_PATCH_QOS_ENABLE
int sysctl_qos_enable_flag;
#endif
...
#ifdef CONFIG_EFM_PATCH_QOS_ENABLE
        if(sysctl_qos_enable_flag) return FALSE;
#endif
...
#ifdef CONFIG_DRIVERLEVEL_REAL_IPCLONE
extern u32 ipclone_virtual_ip[2];
#endif
...
#ifdef CONFIG_DRIVERLEVEL_REAL_IPCLONE
        if(ipclone_virtual_ip[0] && manip)
        {
                if( (manip->src.u3.ip == htonl(ipclone_virtual_ip[0])) ||
                    (manip->dst.u3.ip == htonl(ipclone_virtual_ip[0]))
                )
                        return;
        }

        if(ipclone_virtual_ip[1] && manip)
        {
                if((manip->src.u3.ip == htonl(ipclone_virtual_ip[1])) ||
                    (manip->dst.u3.ip == htonl(ipclone_virtual_ip[1]))
                )
                        return;
        }
#endif
