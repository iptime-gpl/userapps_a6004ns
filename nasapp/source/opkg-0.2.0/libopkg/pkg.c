...
#ifdef EFM_PATCH
	if (pkg->config_file)
		free (pkg->config_file);
	pkg->config_file = NULL;
#endif // EFM_PATCH
}
...
pkg_formatted_field(FILE *fp, pkg_t *pkg, const char *field)
{
...
#ifdef EFM_PATCH
	  } else if (strcasecmp(field, "Config") == 0) {
	       if (pkg->config) {
                   fprintf(fp, "Config: yes\n");
	       }
	  } else if (strcasecmp(field, "ConfigFile") == 0) {
	       if (pkg->config_file) {
     ...
     case 'n':
     case 'N':
	  if (strcasecmp(field, "NasVersion") == 0) {
	       if (pkg->nas_version) {
                   fprintf(fp, "NasVersion: %s\n", pkg->nas_version);
	       }
	  }
	  break;
#endif // EFM_PATCH
...
#ifdef EFM_PATCH
	  } else if (strcasecmp(field, "Runnable") == 0) {
	       if (pkg->runnable) {
                   fprintf(fp, "Runnable: yes\n");
	       }
#endif // EFM_PATCH
	  } else {
	       goto UNKNOWN_FMT_FIELD;
