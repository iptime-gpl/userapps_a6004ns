...
#ifdef EFM_PATCH
		else if ((mask & PFM_EFM_CONFIG) && is_field("Config", line)) {
			char *tmp = parse_simple("Config", line);
			if (strcmp(tmp, "yes") == 0)
				pkg->config = 1;
			free(tmp);
		}
		else if ((mask & PFM_EFM_CONFIG) && is_field("ConfigFile", line)) {
			pkg->config_file= parse_simple("ConfigFile", line);
		}
#endif // EFM_PATCH
...
#ifdef EFM_PATCH
	case 'N':
		if((mask & PFM_EFM_NAS_VERSION) && is_field("NasVersion", line)) {
			pkg->nas_version = parse_simple("NasVersion", line);
		}
		break;
#endif // EFM_PATCH
	case 'P':
...
#ifdef EFM_PATCH
		else if((mask & PFM_EFM_RUNNABLE) && is_field("Runnable", line)) {
			char *tmp = parse_simple("Runnable", line);
			if (strcmp(tmp, "yes") == 0)
				pkg->runnable = 1;
			free(tmp);
		}
...
#ifdef EFM_PATCH
	case 'U':
		if ((mask & PFM_EFM_UI) && is_field("UI", line)) {
			char *tmp = parse_simple("UI", line);
			if (strcmp(tmp, "yes") == 0)
				pkg->ui = 1;
			free(tmp);
		}
