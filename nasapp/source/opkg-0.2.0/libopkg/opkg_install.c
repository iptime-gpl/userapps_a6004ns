...
#ifdef EFM_PATCH
	       else // if (dep->state_status == SS_INSTALLED)// find upgradable dep pkg.
	       {
		       pkg_t *new;
		       int cmp;
		       char *dep_version, *new_version;

		       new = pkg_hash_fetch_best_installation_candidate_by_name(dep->name);
		       if (new)
			       //opkg_upgrade_pkg(dep);
		       {
			       dep_version = pkg_version_str_alloc(dep);
			       new_version = pkg_version_str_alloc(new);

			       cmp = pkg_compare_versions(dep, new);
			       opkg_msg(DEBUG, "Comparing visible versions of pkg %s:"
					       "\n\t%s is installed "
					       "\n\t%s is available "
					       "\n\t%d was comparison result\n",
					       dep->name, dep_version, new_version, cmp);
			       ...
			       if (cmp < 0) {
				       new->dest = dep->dest;
				       dep->state_want = SW_DEINSTALL;

				       new->state_flag |= SF_USER;

				       opkg_msg(DEBUG2,"Calling opkg_install_pkg.\n");
				       err = opkg_install_pkg(new, 1);
				       dep->auto_installed = 1;
				       if (err) {
					       pkg_vec_free(depends);
					       return err;
