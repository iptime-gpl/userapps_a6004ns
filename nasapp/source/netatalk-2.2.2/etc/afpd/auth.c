...
static int login(AFPObj *obj, struct passwd *pwd, void (*logout)(void), int expired)
{
...
#ifdef USE_EFMLOG // for tally	
	check_tally_auth(TALLY_SERVICE_AFP, TALLY_AUTH_FAIL, efmlog_session);
#endif // USE_EFMLOG
    ...
    // LOGIN
    char log_user[64];
    sprintf(log_user, "%s(%s)", pwd->pw_name, efmlog_session);
    sprintf(log_user, "%s(%s)", !strcmp(pwd->pw_name, "nobody")?(char *)(EFMLOG_TEXT(efmlog_locale, "AFP_GUEST")):pwd->pw_name, 
			    efmlog_session);
    efmlog( efmlog_locale, LOG_EFM, EFMLOG_AFP_USER_LOGIN_SUCCESS, log_user, getpid(), 
		    (char *)(EFMLOG_TEXT(efmlog_locale, "AFP_USER_LOGIN_SUCCESS")));
    //LOG(log_note, logtype_afpd, "==================== %s(%d) : log_user[%s]", __FUNCTION__, __LINE__, log_user);
	check_tally_auth(TALLY_SERVICE_AFP, TALLY_AUTH_SUCCESS, efmlog_session);
#endif // USE_EFMLOG
...
#if USE_EFMLOG // for tally
    if (check_tally_auth(TALLY_SERVICE_AFP, TALLY_AUTH_CHECK, efmlog_session))
    {
	    efmlog_simple( EFMLOG_AFP_LOGIN_FAIL, SYSTEM_ACCOUNT, "AFP_LOGIN_BLOCKED", efmlog_session);
...
int afp_logout(AFPObj *obj, char *ibuf _U_, size_t ibuflen  _U_, char *rbuf  _U_, size_t *rbuflen)
{
    ...
    sprintf(log_user, "%s(%s)", !strcmp(obj->username, "nobody")?(char *)(EFMLOG_TEXT(efmlog_locale, "AFP_GUEST")):obj->username, 
			    efmlog_session);
    efmlog( efmlog_locale, LOG_EFM, EFMLOG_AFP_USER_LOGOUT,
		    log_user, getpid(), (char *)(EFMLOG_TEXT(efmlog_locale, "AFP_USER_LOGOUT")));
    //LOG(log_note, logtype_afpd, "==================== %s(%d) : log_user[%s]", __FUNCTION__, __LINE__, log_user);
#endif // USE_EFMLOG
