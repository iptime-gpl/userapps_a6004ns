...
#ifdef USE_EFMLOG
	    // Move and Rename FILE/DIR
	    if (oldname[0] != '.' && newname[0] != '.')
	    {
		    if (isdir)
		    {
			    efmlog( efmlog_locale, LOG_EFM, EFMLOG_AFP_RENAME_DIR, log_user, getpid(), (char *)(EFMLOG_TEXT(efmlog_locale, "AFP_RENAME_DIR")), 
					    !strncmp((char *)sdir->d_fullpath->data, "/mnt", 4)?(char *)sdir->d_fullpath->data+4:(char *)sdir->d_fullpath->data, 
					    !strncmp((char *)curdir->d_fullpath->data, "/mnt", 4)?(char *)curdir->d_fullpath->data+4:(char *)curdir->d_fullpath->data, 
					    upath);
		    }
		    else
		    {
			    efmlog( efmlog_locale, LOG_EFM, EFMLOG_AFP_RENAME_FILE, log_user, getpid(), (char *)(EFMLOG_TEXT(efmlog_locale, "AFP_RENAME_FILE")), 
					    !strncmp(oldunixname, "/mnt", 4)?oldunixname+4:oldunixname,
					    !strncmp((char *)curdir->d_fullpath->data, "/mnt", 4)?(char *)curdir->d_fullpath->data+4:(char *)curdir->d_fullpath->data, 
					    upath);
		    }
		    //LOG(log_note, logtype_afpd, "==================== %s(%d) : log_user[%s]", __FUNCTION__, __LINE__, log_user);
	    }
#endif // USE_EFMLOG
        if (stat(upath, st) < 0) {
            rc = AFPERR_MISC;
...
#ifdef USE_EFMLOG
    sprintf(log_user, "%s(%s)", !strcmp(obj->username, "nobody")?(char *)(EFMLOG_TEXT(efmlog_locale, "AFP_GUEST")):obj->username, 
			    efmlog_session);
#endif // USE_EFMLOG
...
int afp_delete(AFPObj *obj, char *ibuf, size_t ibuflen _U_, char *rbuf _U_, size_t *rbuflen)
{
    ...
    *rbuflen = 0;
    ...
    if (path_isadir(s_path))
    {
	    char *efmlog_str = (char *)s_path->d_dir->d_u_name->data;
	    if (efmlog_str[0] != '.')
	    {
		    efmlog( efmlog_locale, LOG_EFM, EFMLOG_AFP_RM_DIR, log_user, getpid(), (char *)(EFMLOG_TEXT(efmlog_locale, "AFP_RM_DIR")), 
				    !strncmp((char *)dir->d_fullpath->data, "/mnt", 4)?(char *)dir->d_fullpath->data+4:(char *)dir->d_fullpath->data, 
				    efmlog_str);
	    }
    }
    else
    {
	    if (upath[0] != '.')
	    {
		    efmlog( efmlog_locale, LOG_EFM, EFMLOG_AFP_RM_FILE, log_user, getpid(), (char *)(EFMLOG_TEXT(efmlog_locale, "AFP_RM_FILE")), 
				    !strncmp((char *)dir->d_fullpath->data, "/mnt", 4)?(char *)dir->d_fullpath->data+4:(char *)dir->d_fullpath->data, 
				    upath);
	    }
    }
