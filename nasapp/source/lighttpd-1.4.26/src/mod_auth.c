...
int auth_check_nasconfig(connection *con)
{
        char remoteip[32];
        char session_id[128];
        char *ptr, *cookie;
        ...
        if(strncmp(con->uri.path->ptr,"/nasconfig.set",strlen("/nasconfig.set")))
                return 1;
        ...
        strcpy(remoteip,inet_ntoa(con->dst_addr.ipv4.sin_addr));

        return check_http_session("Mgmt",session_id,remoteip);
}
#endif


#if defined EFM_PATCH || defined USE_ROUTER_NAS



int check_process_remote_addr(process_t *prss, char *ipaddr)
{
#if	0
        sock_info_t *ski;

        syslog(LOG_DEBUG,"Check 1 : %d\n", prss->tcp_socklist.count);
        for(ski=prss->tcp_socklist.head;ski;ski=ski->next)
        {
        syslog(LOG_DEBUG,"Check SocketList -> %s %s\n", ski->remote_addr,ipaddr);
                if(!strcmp(ski->remote_addr,ipaddr))
                {
                        //syslog(LOG_DEBUG,"----> remote_addr: %s ... %s", ski->remote_addr, ipaddr);
                        return 1;
                }
        }
#else
	if(strstr(prss->cmdline,ipaddr))
		return 1;
#endif
        return 0;
}

/* Because Android don't support COOKIE for http streaming. We removed it */  
#define NO_USERID_COOKIE_SUPPORT 1
int auth_check_http_streaming(connection *con)
{
        list_head_t proclist;
        process_t *prss;
        char *authdir;
        int n, authflag=0;
        char log_userid[256],user_id[128], remoteip[32];
        char *log_filename,*userid,*ptr;
#ifndef NO_USERID_COOKIE_SUPPORT
	char *cookie;
	data_string *ds;
#endif

	if(strncmp(con->uri.path->ptr,"/httpstream",strlen("/httpstream")))
		return 1;
	...
	if(!cookie)
		return 0;
#endif
        authdir = strdup(con->uri.path->ptr);
	if(!authdir)
		return 0;

	ptr=inet_ntoa(con->dst_addr.ipv4.sin_addr);
	if(!ptr) return 0;
	strcpy(remoteip,ptr);
	// syslog(LOG_DEBUG,"mod_auth ----------------> %s , %s , %s, ", authdir, cookie, remoteip);
	// syslog(LOG_DEBUG,"http_range ----------------> %s", con->request.http_range);

        userid=strtok(&authdir[1],"/");
        if(!userid)
	{
		free(authdir);
		return 0;
	...
	if(!log_filename)
	{
        ...
        sprintf(log_userid,"%s(%s)",userid, remoteip);

#ifndef NO_USERID_COOKIE_SUPPORT
	// syslog(LOG_DEBUG,"log_filename ----------------> %s, %s", log_filename, remoteip);
        if(strcmp(&cookie[strlen("username=")],userid))
        {
                efmlog_simple( EFMLOG_IPDISK_ACCESS_STREAM_FAIL, log_userid, "HTTP_Stream_Access_Fail2", log_filename);
		free(authdir);
                return 0;
        }
#endif

        if(con->request.http_range && strncmp(con->request.http_range,"0-", 2))
	{
		// syslog(LOG_DEBUG,"Range Success -> ");
		free(authdir);
		return 1;
	}

        n=get_process_list("proftpd", &proclist, 1);
        for( prss=proclist.head; prss; prss=prss->next)
        {
		// syslog(LOG_DEBUG,"check proftpd : %s, %s  ------- remoteip: %s", userid,prss->user, remoteip);
                if(!strcmp(userid,prss->user) && check_process_remote_addr(prss,remoteip))
                {
#ifdef EFM_PATCH 
                        efmlog_simple( EFMLOG_IPDISK_ACCESS_STREAM_OK, log_userid, "HTTP_Stream_Access_OK", log_filename);
#endif
                        //syslog(LOG_DEBUG,"Auth OK .. <%s>:<%s>:<%s>", prss->user,userid, httpd_ntoa( &hc->client_addr ));
                        authflag=1;
                        break;
                }
        }
        ...
        syslog(LOG_DEBUG,"HTTP streaming auth [%s]-[%s]->[%s]", log_userid, con->uri.path->ptr ,(authflag==1)?"OK":"Fail");

#ifndef USE_ROUTER_NAS
        if(!authflag)
                efmlog_simple( EFMLOG_IPDISK_ACCESS_STREAM_FAIL, log_userid, "HTTP_Stream_Access_Fail", log_filename);
#endif
...
static handler_t mod_auth_uri_handler(server *srv, connection *con, void *p_d) {
...
#ifdef EFM_PATCH
	if(get_hdd_sleep_status())
	{
		if(
		    strncmp(con->uri.path->ptr,"/image/",strlen("/image/")) &&
		    strncmp(con->uri.path->ptr,"/css/",strlen("/css/")) &&
		    strncmp(con->uri.path->ptr,"/cgi/",strlen("/cgi/")) &&
		    strncmp(con->uri.path->ptr,"/javascript/",strlen("/javascript/")) &&
		    strcmp(con->uri.path->ptr,"/loading_page.cgi") &&
		    strcmp(con->uri.path->ptr,"/status.cgi") &&
		    strcmp(con->uri.path->ptr,"/checkauth.cgi") &&
		    strcmp(con->uri.path->ptr,"/index.html") &&
		    strcmp(con->uri.path->ptr,"/") &&
		    strcmp(con->uri.path->ptr,"/login/login.cgi") &&
		    strcmp(con->uri.path->ptr,"/favicon.ico") &&   /* sometimes IE request this  */
		    strcmp(con->uri.path->ptr,"/urlnoservice.cgi") &&  
		    strcmp(con->uri.path->ptr,"/login.cgi") &&
		    strcmp(con->uri.path->ptr,"/login_handler.cgi")
				)
		{
			httpd_conf_t httpd_conf;
			get_httpd_config(&httpd_conf);

			if (httpd_conf.webdav_enable == 1)
			{
				if (strstr(con->uri.path->ptr, httpd_conf.webdav_tag) || 
					strstr(con->uri.path->ptr, httpd_conf.webdav_public_tag))
				{
					syslog(LOG_DEBUG,"Wait for waking up HDD ---> %s", con->uri.path->ptr);
					wait_for_hdd_wakeup("HTTP");
				}
			}

			if (httpd_conf.dirlist_enable == 1)
			{
				if (strstr(con->uri.path->ptr, httpd_conf.dirlist_tag) || 
					strstr(con->uri.path->ptr, httpd_conf.dirlist_public_tag))
				{
					syslog(LOG_DEBUG,"Wait for waking up HDD ---> %s", con->uri.path->ptr);
					wait_for_hdd_wakeup("HTTP");
				}
	 ...
	 // syslog(LOG_DEBUG,"--------> %s", con->uri.path->ptr);
	 if(!auth_check_nasconfig(con))
	 {
		 con->http_status = 400;
		 con->mode = DIRECT;
...
#if defined EFM_PATCH || defined USE_ROUTER_NAS
	 if(!auth_check_http_streaming(con))
	 {
		 con->http_status = 400;
		 con->mode = DIRECT;
		 return HANDLER_FINISHED;
	...
	//syslog(LOG_DEBUG,"con->uri.path->ptr: %s", con->uri.path->ptr);
...
#ifdef EFM_PATCH // for tally
	char *remote_ip = inet_ntoa(con->dst_addr.ipv4.sin_addr);
#endif // EFM_PATCH
...
#ifdef EFM_PATCH // for tally
		/* Login block check */
		if (check_tally_auth(TALLY_SERVICE_WEBDAV, TALLY_AUTH_CHECK, remote_ip))
		{
			con->http_status = 401; // forbidden
			con->mode = DIRECT;

			efmlog_simple( EFMLOG_WEBDAV_LOGIN_FAIL, SYSTEM_ACCOUNT, "WEBDAV_LOGIN_BLOCKED", remote_ip);
			return HANDLER_FINISHED;
		}
#endif // EFM_PATCH
...
#ifdef EFM_PATCH // for tally
						check_tally_auth(TALLY_SERVICE_WEBDAV, TALLY_AUTH_FAIL, remote_ip); // Fail
#endif // EFM_PATCH
						return HANDLER_FINISHED;
					}
#ifdef EFM_PATCH // for tally
...
#ifdef EFM_PATCH // for tally
		check_tally_auth(TALLY_SERVICE_WEBDAV, TALLY_AUTH_SUCCESS, remote_ip); // Success
#endif // EFM_PATCH
