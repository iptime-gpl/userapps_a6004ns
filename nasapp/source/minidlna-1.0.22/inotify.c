...
inotify_insert_file(char * name, const char * path)
{
...
#ifdef USE_EFM_PATCH
       update_scan_status(path); 
#endif
	/* Is it cover art for another file? */
	if( is_image(path) )
		update_if_album_art(path);
#ifdef EFM_SMI_SUPPORT
	else if( ends_with(path, ".srt") || ends_with(path, ".smi") ) /* add smi, scchoi */
#else
	...
	if( !depth )
	{
		...
		if( (is_audio(path) || is_playlist(path)) && next_pl_fill != 1 )
		{
			...
			next_pl_fill = time(NULL) + 30; // Schedule a playlist scan for 2 minutes from now.
...
#ifdef EFM_SMI_SUPPORT
	if( ends_with(path, ".srt") || ends_with(path, ".smi") ) /* add smi, scchoi */
#else
	...
	else
	{
		...
		if( (sql_get_table(db, sql, &result, &rows, NULL) == SQLITE_OK) )
		{
			...
			for( i=1; i <= rows; i++ )
			{
				...
				{
#ifdef USE_EFM_PATCH
					// don't delete folder db.
					if (!strncmp(BROWSEDIR_ID, result[i], strlen(BROWSEDIR_ID)) ||
						!strncmp(MUSIC_DIR_ID, result[i], strlen(MUSIC_DIR_ID)) || 
						!strncmp(VIDEO_DIR_ID, result[i], strlen(VIDEO_DIR_ID)) || 
						!strncmp(IMAGE_DIR_ID, result[i], strlen(IMAGE_DIR_ID)))
						continue;

					// don't delete default category.
					if (!strcmp(MUSIC_ID, result[i]) ||
						!strcmp(VIDEO_ID, result[i]) ||
						!strcmp(IMAGE_ID, result[i]) ||
						!strcmp(MUSIC_ALL_ID, result[i]) || 
						!strcmp(MUSIC_GENRE_ID, result[i]) || 
						!strcmp(MUSIC_ARTIST_ID, result[i]) ||
						!strcmp(MUSIC_ALBUM_ID, result[i]) || 
						!strcmp(MUSIC_PLIST_ID, result[i]) ||
						!strcmp(MUSIC_CONTRIB_ARTIST_ID, result[i]) ||
						!strcmp(MUSIC_ALBUM_ARTIST_ID, result[i]) || 
						!strcmp(MUSIC_COMPOSER_ID, result[i]) ||
						!strcmp(MUSIC_RATING_ID, result[i]) ||
						!strcmp(VIDEO_ALL_ID, result[i]) ||
						!strcmp(VIDEO_GENRE_ID, result[i]) || 
						!strcmp(VIDEO_ACTOR_ID, result[i]) ||
						!strcmp(VIDEO_SERIES_ID, result[i]) || 
						!strcmp(VIDEO_PLIST_ID, result[i]) ||
						!strcmp(VIDEO_RATING_ID, result[i]) ||
						!strcmp(IMAGE_ALL_ID, result[i]) ||
						!strcmp(IMAGE_DATE_ID, result[i]) || 
						!strcmp(IMAGE_ALBUM_ID, result[i]) ||
						!strcmp(IMAGE_CAMERA_ID, result[i]) || 
						!strcmp(IMAGE_PLIST_ID, result[i]) ||
						!strcmp(IMAGE_RATING_ID, result[i]))
						continue;

					if (sql_get_int_field(db, "SELECT count(*) from OBJECTS where DETAIL_ID ="
								"(SELECT DETAIL_ID from OBJECTS where OBJECT_ID = '%s')", result[i]) < 2 )
					{
						sql_exec(db, "DELETE from DETAILS where ID ="
...
#ifdef USE_EFM_PATCH // remove album_art info from db.
	char *art_path;
	art_path = strdup(path);
	ptr = strrchr(art_path, '.');
	if (ptr)
		strcpy(ptr, ".jpg");

	sql_exec(db, "DELETE from ALBUM_ART where PATH glob '%q/art_cache%q'", db_path, art_path);
	asprintf(&art_cache, "%s/art_cache%s", db_path, art_path);
	if (art_path) free(art_path);
#else
...
#ifdef USE_EFM_PATCH // remove album_art info from db.
	char * art_cache;
	sql_exec(db, "DELETE from ALBUM_ART where PATH glob '%q/art_cache%q/*'", db_path, path);
	asprintf(&art_cache, "%s/art_cache%s", db_path, path);
	remove(art_cache);
	free(art_cache);
#else
...
#ifdef USE_EFM_PATCH
               update_scan_status(NULL); /* start scan */
#endif
	}
#ifdef USE_EFM_PATCH
	watches = NULL;
